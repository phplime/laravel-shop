    <div id="mainContent">
            ...
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title"><?= __('how_it_works'); ?></h5>
                        <div class="card-tools">
                            <?= __addbtn('', __('add_new'), ['is_sidebar' => 1, 'class' => 'add_works']); ?>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="card-content">
                            <div class="table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <?php foreach ($how_it_works as $key => $row) : ?>
                                            <tr id="hide_<?= _x($row->id); ?>">
                                                ...
                                                <td>
                                                    <?= text_limit(__names($row->_names, 'description', true)); ?>
                                                </td>

                                                <td> <?= __status($row->id, $row->status, $table); ?></td>
                                                <td class="">
                                                    <div class="btnGroup">
                                                        <?= __editBtn('', true, ['is_sidebar' => 1, 'class' => 'edit_works_' . _x($row->id)]); ?>
        ...
        <?= __header(__('add_new'), base_url('admin/home/add_works'), 'add_works'); ?>
        ...
        <?php foreach ($how_it_works as $key => $erow) : ?>
            <?= __header(__('edit'), base_url('admin/home/add_works'), 'edit_works_' . _x($erow->id)); ?>
            ...
            <?php foreach (shop_language() as $lang_key => $lang) : ?>
                ...
                <div class="form-group">
                    <label><?= __('description'); ?> <?= country($lang['country_id'])->flag; ?></label>
                    <textarea name="description[<?= $lang['slug'] ?>]" class="form-control"><?= isset($_details['description']) ? _x($_details['description']) : ''; ?></textarea>
                </div>
            <?php endforeach; ?>
