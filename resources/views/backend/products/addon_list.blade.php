...
<div class="row">
                    ...
                    <div class="table-responsive">
                        <table class='table table-striped table-bordered  data_tables'>
                            ...
                            <tbody id="sortable" class="sortable sorting">
                                @foreach ($extra_list as $key => $extra)
                                <tr id='1'>
                                    <td class="handle">{{ $key+1 }}</td>
                                    <td class="handle"><?= __names($extra, 'title', true) ?></td>
                                    <td>
                                        ...
                                        <?php if (isset($extra->extra_list) && $extra->extra_list->isNotEmpty()): ?>
                                            <table class="table table-striped table-bordered m-0">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <td>{{ $key1+1 }}</td>
                                                        <td><?= __names($ext->addonLibrary, 'addon_name', true) ?></td>
                                                        <td><?= __currency_position(__aExtra($ext, 'price'), $extra->vendor_id) ?></td>
                                                        <td><?= __aExtra($ext, 'max_qty') == 0 ? '&#8734' : __aExtra($ext, 'max_qty'); ?> </td>
                                                        <td class="text-center">
                                                            <a href="javascript:;" onclick="editAssignExtra(`<?= $ext->id ?>`,`<?= __aExtra($ext, 'price') ?>`,`<?= __aExtra($ext, 'max_qty') ?>`)"
                                                                class="btn btn-info btn-sm"> <i class="fa fa-edit"></i>
                                                            </a>
                                                            <?= __deleteBtn($ext->id, 'item_extra_list', false) ?>
                                                        </td>
                                                    </tr>
                                                    @endforeach
                                                </tbody>
                                    ...
                                    <td>
                                        <div class="btnGroup">
                                            <a href="javascript:;" onclick="addExtraModal(`<?= $extra->id ?>`,`<?= __names($extra, 'title') ?>`)"
                                                class="btn btn-secondary btn-sm d-none"><i class="fa fa-plus"></i>
                                                <?= __('add_new') ?> </a>

                                            <a href="javascript:;" onclick="extraListModal(`<?= $extra->id ?>`,`<?= __names($extra, 'title') ?>`)"
                                        ...
                                        <div class="mt-10 btnGroup">
                                            ...
                                            <?= __deleteBtn($extra->id, 'vendor_extra_title_list') ?>
                                        </div>
                                    </td>
                                </tr>
                                @endforeach
    ...
    <div class="col-md-4">
        <div class="card singlePage">
            @include('profile.common_layouts.item_details_thumb',['hideModal'=>true])
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="extrasBody">
                        ...
                        <div class="row">
                            @php
                            $extra_names = $extra->getTranslations()->pluck('title', 'language');
                            @endphp
                            @foreach ($shopLanguages as $lang)
                            <div class="form-group col-md-6">
                                <label><?= __('title') ?> <?= country($lang->country_id)->flag ?></label>
                                <input type="text" name="title[<?= $lang->slug ?>]" class="form-control" value="<?= $extra_names[$lang->slug] ?? '' ?>">
                            </div>
    ...
    <div class="modal-dialog">
        <form action="<?= url('vendor/products/add_library_extras') ?>" method="post"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered w_100p" id="myTable">
                            <thead>
                                <tr>
                                    <td><?= __('title') ?></td>
